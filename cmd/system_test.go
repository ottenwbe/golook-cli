...
var _ = Describe("The system command", func() {
	It("retrieves valid information from the golook server", func() {
		integration.RunPeerInDocker(func(_ *docker.Client, container *docker.Container) {
			...
			testResult := utils.InterceptStdOut(func() {
				systemCmd.Run(nil, nil)
			})
			Expect(testResult).To(ContainSubstring("name"))
			Expect(testResult).To(ContainSubstring("os"))
			Expect(testResult).To(ContainSubstring("uuid"))
		})
	})
})
